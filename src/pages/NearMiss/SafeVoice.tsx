...
title:"안전보이스 보고서",
companyName:"",
documentNumber:`SV_${String(row.id).padStart(8,"0")}`,
createdAt:row.date,
fields:[
...
{label:"미조치 사유",value:row.reason||"-",type:"textarea",section:"content"},
...
const[data,setData]=React.useState<SafeVoiceRow[]>(safeVoiceMockData as SafeVoiceRow[])
const[checkedIds,setCheckedIds]=React.useState<(number|string)[]>([])
const[modalOpen,setModalOpen]=React.useState(false)
const[qrDialogOpen,setQrDialogOpen]=React.useState(false)
const[qrUrl,setQrUrl]=React.useState("")

const{startDate,endDate,searchText,setStartDate,setEndDate,setSearchText,filteredData,handleSearch}=useFilterBar({data,dateKey:"date",searchKeys:["content","registrant"]})
const{currentIndex,handleTabClick}=useTabNavigation(TAB_PATHS)
const{currentPage,totalPages,currentData,onPageChange}=usePagination<SafeVoiceRow>(filteredData as SafeVoiceRow[],30)
const{handleDelete,handleSave,handleExcelDownload,handlePrint,isDownloading,isPrinting}=useHandlers<SafeVoiceRow>({
data:filteredData as SafeVoiceRow[],
checkedIds,
...
const handleSaveRow=(newItem:Partial<SafeVoiceRow>)=>{const nextId=Math.max(...data.map(r=>Number(r.id)))+1;setData(prev=>[{id:nextId,status:"미조치",reason:"",sitePhotos:[],content:"",registrant:"",date:"",...newItem},...prev]);setModalOpen(false)}
