...
export default function PTWSiteEvaluationList() {
  ...
  const { startDate, endDate, searchText, setStartDate, setEndDate, setSearchText, filteredData, handleSearch } = useFilterBar({data, dateKey:"workDate", searchKeys:["workTeam","workerName","author"]})
  const [checkedIds, setCheckedIds] = useState<(number | string)[]>([])
  const { currentPage, totalPages, currentData, onPageChange } = usePagination<SiteEvaluationItem>(filteredData as SiteEvaluationItem[], 30)

  const [qrDialogOpen, setQrDialogOpen] = useState(false)
  const [qrUrl, setQrUrl] = useState("")

  const { handleCreate, handleDelete } = useHandlers({
    data,
    checkedIds,
