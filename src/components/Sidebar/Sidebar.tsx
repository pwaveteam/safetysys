...
<strong className="block text-white font-semibold text-base leading-tight mb-2">{platformName}</strong>
...
<div onClick={()=>{navigate("/mypage");if(onClose)onClose()}}className="flex items-center gap-2 cursor-pointer">
<img src="/ico/ico-avatar.svg"alt="avatar"style={{width:AVATAR_SIZE,height:AVATAR_SIZE,borderRadius:"50%"}}/>
<span className="flex items-center gap-1.5 text-xs text-[#B1B1B1]">
<span className="font-semibold">{currentUser.name}</span> 님
<span className="text-[10px] text-[#999] border border-[#666] rounded-full px-1.5 py-0.5">{currentUser.position}</span>
</span>
</div>
<div className="flex gap-2 mt-1">
<button onClick={()=>{navigate("/business-management");if(onClose)onClose()}}className="flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-full border border-[#C6C6C6] bg-transparent text-white text-xs font-semibold whitespace-nowrap">
...
onClick={(e)=>handleNavClick(e,"/business-management")}
