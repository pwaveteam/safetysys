...
const InfoRow = ({ label, children }: { label: string; children: React.ReactNode }) => (
  <>
    <div className={`font-medium ${TEXT_SECONDARY}`}>{label}</div>
    <div>{children}</div>
  </>
)
      ...
      <div className="bg-white rounded-none md:rounded-2xl w-full md:w-[500px] md:max-w-full p-4 md:p-6 shadow-2xl h-screen md:h-auto md:max-h-[90vh] flex flex-col overflow-y-auto">
        ...
        <div className="space-y-4">
          <div className="bg-gray-50 rounded-lg p-4">
            <div className={`grid grid-cols-[100px_1fr] gap-y-3 ${TEXT_SIZE_TD}`}>
          ...
          <div>
            <p className={`${TEXT_SIZE_TD} font-medium text-gray-700 mb-2`}>결재내용</p>
            <textarea
              value={content}
              onChange={(e) => setContent(e.target.value)}
              placeholder="결재내용을 입력하세요"
              className={`w-full rounded-lg p-3 ${TEXT_SIZE_TD} bg-white ${TEXT_PRIMARY} border ${BORDER_CLASS} placeholder:text-gray-400 focus:outline-none focus:border-gray-400 resize-none min-h-[120px]`}
            />
          </div>
