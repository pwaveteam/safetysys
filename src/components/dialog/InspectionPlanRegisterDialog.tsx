...
interface InspectionPlanRegisterDialogProps {
open: boolean
onClose: () => void
...
export default function InspectionPlanRegisterDialog({ open, onClose, onSave }: InspectionPlanRegisterDialogProps) {
...
location: { required: true, message: "점검장소를 입력하세요" },
field: { required: true, message: "점검분야를 선택하세요" },
kind: { required: true, message: "점검종류를 선택하세요" },
scheduleStart: { required: true, message: "시작일을 선택하세요" },
scheduleEnd: { required: true, message: "종료일을 선택하세요" },
templateName: { required: true, message: "점검표를 선택하세요" }
}), [])
...
const { validateForm, isFieldInvalid, clearErrors } = useForm(validationRules, formValues)
