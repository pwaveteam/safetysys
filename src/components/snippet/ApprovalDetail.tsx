...
if (status === "결재완료") return <Badge color="blue">결재완료</Badge>
if (status === "결재대기") return <Badge color="orange">결재대기</Badge>
if (status === "결재중") return <Badge color="green">결재중</Badge>
...
{isApproved && <Check size={16} className="text-blue-600" />}
{isRejected && <X size={16} className="text-red-600" />}
</div>
</InfoRow>
</div>
</div>
<button className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 border border-gray-200 rounded-lg p-3 w-full">
<FileText size={16} /><span>내용보기</span>
</button>
{isPending && (
...
<div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center"><Check size={20} className="text-blue-600" /></div>
<div>
<p className="text-sm font-medium text-blue-700">결재 완료</p>
<p className="text-xs text-blue-600">이 문서는 승인 처리되었습니다.</p>
</div>
</div>
)}
{isRejected && (
...
const isInProgress = row.status === "결재중"
const isWaiting = row.status === "결재대기"
...
<InfoRow label="결재유형">{row.document}</InfoRow>
<InfoRow label="결재내용">{row.content}</InfoRow>
<InfoRow label="상태">
<div className="flex items-center gap-2">
<StatusBadge status={row.status} />
{isComplete && <Check size={16} className="text-blue-600" />}
{isRejected && <X size={16} className="text-red-600" />}
</div>
</InfoRow>
...
<div>
<p className="text-sm font-medium text-blue-700">결재 완료</p>
<p className="text-xs text-blue-600">모든 결재가 완료되었습니다.</p>
</div>
</div>
)}
{isRejected && (
...
<div>
<p className="text-sm font-medium text-green-700">결재 진행중</p>
<p className="text-xs text-green-600">현재 {row.progress} 단계 진행중입니다.</p>
</div>
</div>
)}
{isWaiting && (
<div className="bg-orange-50 border border-orange-200 rounded-lg p-4 flex items-center gap-3">
<div className="w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center"><Clock size={20} className="text-orange-600" /></div>
<div>
<p className="text-sm font-medium text-orange-700">결재 대기</p>
<p className="text-xs text-orange-600">결재자의 승인을 기다리고 있습니다.</p>
</div>
</div>
)}
</div>
